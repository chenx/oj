...
There is no "auto" keyword like in C++. The alternative is "var". 

A var is a local variable initialized with value, and cannot be null.
...
```
StringBuffer: thread-safe, synchronized; lower in performance
StringBuilder: not threadsafe; faster in performance. Good for single-threaded program
```
...
// Add 1 to an entry. Both work, prefer the 2nd one:
...
// Increment value with default value
...
// Iterate through a set by for-loop.
...
void test() {
        String s = "abc";
        int len = s.length();
        s = s.substring(0, s.length() - 2); // remove last element
        ...
        // Throws-exception versions: add(e), remove(), element()
        // Returns-special-value versions: offer(e), poll(), peek() (returning null or false on failure).
...
## Summary of Core Built-in Data Structure Implementations

| Data Structure Type | Standard Java Interface | Standard Java Class |
|--------------------|------------------------|--------------------|
| Dynamic Array | `List<E>` | `ArrayList<E>` |
| Linked List | `List<E>` / `Deque<E>` | `LinkedList<E>` |
