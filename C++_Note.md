...
Default is MaxHeap in C++, and minHeap in Java and Python.
...
### Anonymous function, lambda function, closure

In C++, the terms lambda function and anonymous function are often used interchangeably to refer to the same feature. However, there is a technical distinction in how C++ implements them compared to traditional functions.

While they describe the same practical tool, C++ lambdas are technically more complex than a "function without a name": 

- Underlying Type: A C++ lambda is actually a functor (an anonymous class object) generated by the compiler. The compiler creates a unique, unnamed class that overloads the operator().
- Closures: Unlike simple functions, lambdas can capture variables from their surrounding scope (the "capture list" []). This combination of a function and its captured environment is formally called a closure.
- Function Pointers: A stateless lambda (one with an empty []) can be implicitly converted into a regular C++ function pointer. Once it has captures, it can no longer be converted to a simple function pointer because it carries additional state.
...
In C++, multithreading support was introduced in C++11 through the <thread> header file.
