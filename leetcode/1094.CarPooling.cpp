...
public:
    bool carPooling(vector<vector<int>>& trips, int capacity) {
        map<int, int> map; // <stop, psg_change>
        for (auto e : trips) {
            map[e[1]] += e[0];
            map[e[2]] -= e[0];
        }
...
numPassengersi passengers and the locations to pick them up and drop them off are fromi and toi respectively. The locations are 
