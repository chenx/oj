...
// This works too and is more clear. From hint.
class Solution2 {
public:
    int countPrimes(int n) {
...
// This works too. Use the commented out version also good, which equates Solution2.
class Solution3 {
public:
    int countPrimes(int n) {
        if (n <= 2) return 0;
...
Hint:
    ...
    we can immediately cut the total iterations half by dividing only 
    ...
    all prime numbers up to n. But don't let that name scare you, I promise 
    that the concept is surprisingly simple.
    ...
    such as 5 × 2 = 10, 5 × 3 = 15, 5 × 4 = 20, 5 × 5 = 25, ... can be marked off. 
    ...
    5 × 2 = 10 was already marked off by multiple of 2, similarly 5 × 3 = 15 
    was already marked off by multiple of 3. Therefore, if the current number is p, 
    ...
    It is easy to say that the terminating loop condition is p < n, which is 
    certainly correct but not efficient. Do you still remember Hint #3?

    Yes, the terminating loop condition can be p < √n, as all non-primes ≥ √n 
    must have already been marked off. When the loop terminates, all the numbers 
    in the table that are non-marked are prime.

    The Sieve of Eratosthenes uses an extra O(n) memory and its runtime complexity 
    is O(n log log n). For the more mathematically inclined readers, you can read 
    more about its algorithm complexity on Wikipedia.
