// From: https://leetcode.com/problems/find-servers-that-handled-most-number-of-requests/editorial/
...
public:
    vector<int> busiestServers(int k, vector<int>& arrival, vector<int>& load) {
        ...
        set<int> free;
        priority_queue<vector<int>, vector<vector<int>>, greater<vector<int>>> busy; // maxHeap. <start, serverId>

        for (int i = 0; i < k; ++ i) free.insert(i); // all servers start free.

        for (int i = 0; i < arrival.size(); ++ i) {
            int start = arrival[i];

            while (! busy.empty() && busy.top()[0] <= start) {
                int serverId = busy.top()[1];
                busy.pop();
            ...
            if (! free.empty()) {
                ...
                int serverId = *ptr;
                free.erase(serverId);
                busy.push({start + load[i], serverId});
                count[serverId] ++;
            }
        }
        ...
        for (int i = 0; i < k; ++ i) {
            if (count[i] == maxCount) result.push_back(i);
        }
        return result;
    }
};
...
Each server has infinite computational capacity but cannot handle more than one request at a time. The 
