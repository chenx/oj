...
public:
    int numUniqueEmails(vector<string>& emails) {
        ...
        for (string& email : emails) {
            result.insert(processEmail(email));
        }
        return result.size();
    }
    ...
    string processEmail(string& email) {
        ...
        string localName = email.substr(0, indexAt);
        string suffix = email.substr(indexAt);

        string actualLocalName;
        for (char ch : localName) {
            if (ch == '+') break;
            if (ch != '.') actualLocalName += ch;
        }
        return actualLocalName + suffix;
    }
};
