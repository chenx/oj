...
public:
    int minimumOperationsToWriteY(vector<vector<int>>& grid) {
        ...
        int yCells[3] = {0}, nonYCells[3] = {0};
        for (int i = 0; i < n; ++ i) {
            for (int j = 0; j < n; ++ j) {
                if (i == j && i <= n/2) ++ yCells[grid[i][j]];
                else if (i+j == n-1 && i <= n/2) ++ yCells[grid[i][j]];
                else if (j == n/2 && i >= n/2) ++ yCells[grid[i][j]];
                else ++ nonYCells[grid[i][j]];
            }
        }

        int total = n*n, minOp = total;

        for (int i = 0; i < 3; ++ i) {
            for (int j = 0; j < 3; ++ j) {
                if (i != j) {
                    int op = total - yCells[i] - nonYCells[j];
                    minOp = min(minOp, op);
                }
            }
        }
