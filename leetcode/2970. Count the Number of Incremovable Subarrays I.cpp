...
public:
    int incremovableSubarrayCount(vector<int>& nums) {
        ...
        for (int i = 0; i < n; ++ i) {
            for (int j = i; j < n; ++ j) {
                if (inc(nums, i, j)) ++ count;
            }
        }
        return count;
    }

    bool inc(vector<int>& nums, int i, int j) {
        for (int x = 0; x < i; ++ x) {
            if (x+1 < i && nums[x] >= nums[x+1]) return false;
        }
        if (i-1 >= 0 && j+1 < nums.size() && nums[i-1] >= nums[j+1]) return false;
...
Input: nums = [8,7,6,6]
Output: 3
Explanation: The 3 incremovable subarrays are: [8,7,6], [7,6,6], and [8,7,6,6]. Note that [8,7] is not 
an incremovable subarray because after removing [8,7] nums becomes [6,6], which is sorted in ascending 
order but not strictly increasing.
 */
