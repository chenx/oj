// Time: Time Complexity: O(√area)
// Space: O(1)
class Solution {
public:
    vector<int> constructRectangle(int area) {
        // Find the width starting from the square root of the area
        // This ensures L >= W and minimizes |L - W|
        int width = std::sqrt(area);
      
        // Decrease width until we find a divisor of the area
        // This guarantees that area % width == 0
        while (area % width != 0) {  // Time: O(√area) in worst case
            --width;
        }
