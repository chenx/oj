...
class Solution:
    def construct(self, grid: List[List[int]]) -> 'Node':
        def dfs(grid, x, y, size):
            ...
            size //= 2
            a = dfs(grid, x, y, size)
            b = dfs(grid, x, y + size, size)
            c = dfs(grid, x + size, y, size)
            d = dfs(grid, x + size, y + size, size)

            if a.isLeaf and b.isLeaf and c.isLeaf and d.isLeaf and \
                a.val == b.val and b.val == c.val and c.val == d.val:
                return Node(grid[x][y], True)
            
            return Node(grid[x][y], False, a, b, c, d)
...
Besides, each node has two attributes:
    ...
    val: True if the node represents a grid of 1's or False if the node represents a grid 
        of 0's. Notice that you can assign the val to True or False when isLeaf is False, 
...
Quad-Tree using level order traversal, where null signifies a path terminator where no node exists below.
