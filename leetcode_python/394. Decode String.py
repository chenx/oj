...
# We traverse a string of size n and iterate k times to decode each pattern of form k[string]. This gives 
# us worst case time complexity as O(maxK⋅n).
#
# Space Complexity: O(m+n), where m is the number of letters(a-z) and n is the number of digits(0-9) in 
# string s. In worst case, the maximum size of stringStack and countStack could be m and n respectively.
#
class Solution:
    def decodeString(self, s: str) -> str:
        ...
        for ch in s:
            if ch.isdigit():
                ...
                prev = strStack.pop()
                ct = numStack.pop()
                # print(f"prev={prev}, ct={ct}, cur={cur}") # prev=a, ct=2, cur=c
                cur = prev + ct * cur
            else:
                cur += ch
