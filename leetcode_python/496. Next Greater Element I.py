# Time: O(n)
# Space: O(n) for stack and nextGreaterMap.
class Solution:
    def nextGreaterElement(self, nums1: List[int], nums2: List[int]) -> List[int]:
        nextGreaterMap : dict[int, int] = {} # <element in nums2, next greater element>
        stack = []
        for i in range(len(nums2)):
            while len(stack) > 0 and nums2[i] > stack[-1]:
                nextGreaterMap[stack[-1]] = nums2[i]
                stack.pop()
            stack.append(nums2[i])
        
        while stack:
            nextGreaterMap[stack[-1]] = -1
            stack.pop()
        
        ans = []
        for num in nums1:
            ans.append(nextGreaterMap[num] if num in nextGreaterMap else -1)
        return ans
