...
public:
    ...
    void add(vector<ListNode*> & ht, int val) {
        int key = (val >= 0) ? 
                  (val % ht.size()) 
                  : (ht.size() - (-val) % ht.size()); // note this!
        
        ListNode * n = new ListNode(val);
        
        if (ht[key] == NULL) ht[key] = n;
        else {
            ListNode * next = ht[key];
...
For negative integer in hash table, the position can be calculated as follow:
    ...
    C++/Java: if you directly calculate -4 % 3 you will get -1. You can 
    use function: a % b = (a % b + b) % b to make it is a non negative integer.
